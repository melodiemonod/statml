...
tmp = data.frame(method = c(rep("LS", length(D_grid)), rep(paste("Ridge lambda =", round(lambdas, digits = 4)), length(D_grid)) , rep(paste("Lasso lambda =", round(lambdas, digits = 4)), length(D_grid))), 
                 ...
                 MSE_test_i  = log(c(df[grepl( "ls.MSE_test", names(df))], df[grepl( "ridge.MSE_test", names(df))], df[grepl( "lasso.MSE_test", names(df))])),
                 ...
                 MSE_test_l  = log(c(df2[grepl( "ls.MSE_test", names(df2))], df2[grepl( "ridge.MSE_test", names(df2))], df2[grepl( "lasso.MSE_test", names(df2))])),
                 ...
                 L2_norm_i = log(c(df[grepl( "ls.L2_norm", names(df))], df[grepl( "ridge.L2_norm", names(df))], df[grepl( "lasso.L2_norm", names(df))])), 
                 L2_norm_l = log(c(df2[grepl( "ls.L2_norm", names(df2))], df2[grepl( "ridge.L2_norm", names(df2))], df2[grepl( "lasso.L2_norm", names(df2))])))
...
tmp2 = data.frame(method = c(rep("LS", length(D_grid))), 
                  ...
                  MSE_test  = log(c(df3[grepl( "ls.MSE_test", names(df3))])),
                  ZOL_training  = c(df3[grepl( "ls.ZOL_train", names(df3))]),
                  ZOL_test  = c(df3[grepl( "ls.ZOL_test", names(df3))]),
                  L2_norm = log(c(df3[grepl( "ls.L2_norm", names(df3))])))
...
p2.ls = ggplot(tmp[grepl("LS", tmp$method),], aes(x = D, color = method)) +
  geom_line(aes(y = MSE_test_i)) +
  geom_point(aes(y = MSE_test_i), shape = 4) +
...
p4.ls =  ggplot(tmp[grepl("LS", tmp$method),], aes(x = D, color = method)) +
  geom_line(aes(y = L2_norm_i))+
  geom_point(aes(y = L2_norm_i), shape = 4) +
  theme_bw()+
  ylab("Norm") +
  theme(axis.title.x=element_blank(), legend.position = "none")
...
ggsave(plot = plot_ls, w = 6, h = 10, device = "pdf", filename  = file.path(indir,"plot_leastnorm.pdf"))
